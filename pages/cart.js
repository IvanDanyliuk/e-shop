                                            ...
                                            <TableRow key={item._id}>
                                                ...
                                                <TableCell align='right'>
                                                    <Select value={item.quantity}>
                                                        {[...Array(item.countInStock).keys()].map(num => (<MenuItem key={num + 1} value={num + 1}>{num + 1}</MenuItem>))}
                                                    </Select>
                                                </TableCell>
                                                <TableCell align='right'>
                                                    ${item.price}
                                ...
                                <List>
                                    <ListItem>
                                        <Typography variant='h2'>
                                            Subtotal ({cartItems.reduce((acc, cur) => acc + cur.quantity, 0)}{' '} items : ${cartItems.reduce((acc, cur) => acc + cur.quantity * cur.price, 0)})
                                        </Typography>
                                    </ListItem>
                                    <ListItem>
